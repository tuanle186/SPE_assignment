...
class Generator:
    ...
    def generateArrival(self):
        while True:
            request_type, is_leaving = Generator.ProbGen()
            self.customer_id += 1
...
def main(i):
    global logger
    log_path = f'log_tc2/local_{i}.log'
    os.makedirs(os.path.dirname(log_path), exist_ok=True)
    with open('log_tc2/local_{}.log'.format(i),'w') as file:
        pass
    ...
    # Create ArrivalEvent
    generator = Generator(env, arrival_rate, service_rate_router, service_rate_dept, num_customer_router)
